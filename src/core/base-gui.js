...
export default class BaseGUI {
  ...
  showAfterAssetsLoad() {
    ...
    if (BASE_CONFIG.camera.enableOrbitControls) {
  ...
  _setupCameraGUI() {
    const guiHelpersFolder = GUIHelper.getFolder(GUIFolders.Camera);

    guiHelpersFolder.add(this._camera.position, 'x', -30, 30)
      .name('Position X');

    guiHelpersFolder.add(this._camera.position, 'y', 2, 40)
      .name('Position Y');

    guiHelpersFolder.add(this._camera.position, 'z', -30, 30)
      .name('Position Z');

    this._cameraLookAt = BASE_CONFIG.camera.lookAt;

    guiHelpersFolder.add(this._cameraLookAt, 'x', -10, 10)
      .name('Look at X')
      .onChange(() => this._camera.lookAt(this._cameraLookAt.x, this._cameraLookAt.y, this._cameraLookAt.z));

    guiHelpersFolder.add(this._cameraLookAt, 'y', 0, 10)
      .name('Look at Y')
      .onChange(() => this._camera.lookAt(this._cameraLookAt.x, this._cameraLookAt.y, this._cameraLookAt.z));

    guiHelpersFolder.add(this._cameraLookAt, 'z', -10, 10)
      .name('Look at Z')
      .onChange(() => this._camera.lookAt(this._cameraLookAt.x, this._cameraLookAt.y, this._cameraLookAt.z));
  }
